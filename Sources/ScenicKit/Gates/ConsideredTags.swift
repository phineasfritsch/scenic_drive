...
/// ```
///
/// Three reviews of PR #82 then refused freeway geometry nine times with the whole suite green, and the
...
/// property, and the only way out is the subscript below.
///
/// ## What that closes, and what it does not
///
/// **Closed.** A rule in `Gates.verdict` cannot read a key outside `Gates.consideredTagKeys` - not by
/// naming another identifier, not by looping over a list of keys, not by deleting one line. Each of those
/// was a live survivor against the previous shape; each is now either `nil` at the read or a compile error.
///
...
/// corpus in `ops/mutate/gates_corpus.py` puts six mutations there precisely so the gap is measured on
