...
- 2026-09-18 round-1 fixes by agent/claude-opus-5, acting for owner agent/claude-fable-5-1 on the owner's
  ...
  * **STILL OPEN, not fixed by this round.** (a) Nothing here was compiled and this PR still cannot prove it
    ...
    record either way.
- 2026-09-18 review round 2 by agent/rv2-pr88 (PR #88, head `5aeaf54`, own throwaway worktree `.worktrees/rv2-pr88`, detached at that sha and removed at the end; SPM scratch under it, deleted). **PASS on substance; the sign-off transition is BLOCKED and was not made â€” see the last paragraph.** Eleven subject files hashed against `git show 5aeaf54:<path>` before measuring and re-hashed after every mutating run, all matching; `ops/lib/check-line-cap` 100755 and `ops/lib/check-pbxproj-graph.py` 100644 at the tree, as claimed. `.worktrees/T-0141` `git status --short` empty and HEAD == `origin/task/T-0141` == `5aeaf54`.
  **All nine acceptance lines reproduce character for character**, every exit code taken bare from the process, never through a pipe: `git diff --stat origin/main -- Package.swift` prints nothing and `grep -cE '^ *[.]package[(].*apps/' Package.swift` -> `0`, exit 1 as the line itself warns (the round-0 `grep -c apps/ Package.swift` -> `1`, and `grep -n apps/ Package.swift` shows the hit is `Package.swift:4`, a comment); from `apps/ios/Packages/ScenicApp` under its own `--scratch-path`, `swift package dump-package` emits the fileSystem dependency as `"nameForTargetDependencyResolutionOnly": "ScenicDrive"` at the checkout root and `swift package resolve` -> `Working copy of https://github.com/maplibre/maplibre-gl-native-distribution resolved at 6.31.0`, exit **0** (the identity reads `rv2-pr88` here, the directory name, exactly the caveat the fixer recorded; the untracked `Package.resolved` it wrote was deleted and the tree re-hashed); `bash ops/lib/check-line-cap` -> `P-SRC-02: 48 Swift files tracked (Sources=17, Tests=23, apps/ios=8), none over 300 lines`, exit 0; the one-type-per-file pipeline -> `1 0` and `7 1`; `bash ops/check-pins --source-only` -> `PINS ok=8 skipped=11 pending=1 expired=0 failed=0 tier=linux source-only`, exit 0 and `bash ops/queue-check` -> `QUEUE OK (140 tasks)`, exit 0, both bare; `python ops/lib/check-pbxproj-graph.py` -> `pbxproj-graph: 28 assertions, 0 failed`, exit 0, and on the green run the location assertion's detail column reads `absent` (F10 is really fixed); `gh api repos/maplibre/maplibre-gl-native-distribution/releases --jq '.[0:5][].tag_name'` -> 6.31.0 6.30.0 6.29.0 6.28.0 6.27.0 with `apps/ios/Packages/ScenicApp/Package.swift:45` pinning exact 6.31.0; the five Nominatim waypoints and the NOT-COMPILED line are record claims and are true of the tree.
  **Four re-breaks, each red by the NAME of what it broke, each restored and re-hashed.** (1) `git rm --cached $(git ls-files 'apps/ios/**/*.swift')` -> `P-SRC-02: root(s) matching no tracked .swift file: apps/ios` / `Population: Sources=17, Tests=23, apps/ios=0 (total 40).`, exit 1; `git reset HEAD -- apps/ios` -> green again at 48. (2) `objectVersion = 77` -> `60`: `FAIL objectVersion is 77   60`, exit 1. (3) product rename: `FAIL every packageProductDependency is a declared product   target imports ['FeatureScenicHome']; Package.swift declares ['DesignSystem', 'MapAdapter', 'FeatureScenicHomeZZ']`, exit 1. (4) `NSLocationWhenInUseUsageDescription` re-added: `FAIL ... absent because nothing asks for location  present`, exit 1 â€” green says `absent`, red says `present`, so the column proves something. And the blocking defect at its source: `path: "../../.."` put back -> `swift package resolve` -> `error: the package manifest at '...\apps\Package.swift' cannot be accessed`, exit 1.
  **Six attacks round 1 did not run, all clean.** The buildable folder's `PBXFileSystemSynchronizedBuildFileExceptionSet` really lists `membershipExceptions = (Info.plist,)` and points at target `08CC3078A869E795EA1EFD38`; the scheme's three `BlueprintIdentifier`s are that same id, defined as the PBXNativeTarget at `project.pbxproj:69`; `apps/ios/ci_scripts/ci_post_clone.sh` is 100755, sits beside the `.xcodeproj`, and under `set -eu` only prints toolchain versions and `${CI_*:-?}` defaults; `gh api .../releases/latest` -> `6.31.0 prerelease=false`, so the exact pin is the current release AND the tag the manifest names; the root package is `name: "ScenicDrive"` with products `ScenicKit` and `Handoff`, exactly the two the Apple manifest asks for at :85-86. The residual hole the fixer STATES is stated and not hidden â€” and real: a tracked 321-line `apps/ios/Hole.swift` left `check-line-cap` printing `48 ... none over 300 lines`, exit 0. It is in the script header and in STILL OPEN (c). Removed; tree clean. No comment or Log line carries a count no command printed: `263` matches `awk 'END{print NR}'`, `28 assertions` is `len(results)` and not a literal, `grep -c P-ATTR-01 pins/PINS.yaml` -> 0 and `git ls-files | grep -ic xcuitest` -> 0, the 3.35:1 recomputes (0.244999 vs 0.938967 -> 3.3524), `git ls-files 'Sources/**/*.swift' 'Tests/**/*.swift' | wc -l` -> 40, and `bash ops/lib/check-exec-bits` -> `P-OPS-01: 45 files, 23 required present, all modes correct`, exit 0.
  **Record.** `git diff 32355c5..5aeaf54` over this file removes exactly 9 lines â€” the `touches:` line and the eight round-0 acceptance lines â€” and nothing else; every Log line, including reviewer agent/rv1-pr88's round-1 entry, is present unedited, and the Brief's path spec is corrected by an appended dated bracket beside the original rather than over it.
  **Ruling on STILL OPEN (b), `ops/test` not run: NOT blocking**, though `verify:` names it. `ops/test:23` runs `swift test --parallel --xunit-output ...` with no `--scratch-path`, into the shared `.build` that CLAUDE.md forbids on a shared box; `git show --stat 5aeaf54` touches only `apps/ios/**`, `ops/lib/**` and `queue/**`, so no Linux target moved; and `.github/workflows/linux-core.yml:64-67` runs `bash ops/test` and the full `bash ops/check-pins` in the pinned image, where `gh pr checks 88` reports `core pass` and `pins-source-only pass`. The gate is satisfied by CI, not skipped.
  **Two non-blocking notes for the merger, neither a false claim.** (i) `ops/lib/check-pbxproj-graph.py` is invoked by nothing mechanical: `grep -rn check-pbxproj-graph` finds it only in itself and in this task file, it is not an assertion in `pins/PINS.yaml` and not a step in `linux-core.yml`, so after merge the pbxproj graph is proved only by someone running the acceptance line. Wiring it to a pin belongs to [[T-0009]] or a follow-up. (ii) STILL OPEN (d) stands: the [[T-0010]] and [[T-0037]] moves to `done/` are the merger's.
  **The transition was NOT made, and this entry is not committed by me.** With `state: done`, `reviewer: agent/rv2-pr88` and the `git mv` to `queue/done/` applied in `.worktrees/T-0141`, `bash ops/queue-check` run bare refused: `QUEUE CHECK FAIL` / `queue/LOCKS/package-swift.lock held by T-0141, which is not in claimed/` / `queue/LOCKS/pbxproj.lock held by T-0141, which is not in claimed/`, exit 1. Both locks read `T-0141 agent/claude-fable-5-1 2026-09-18T15:45:37Z`, and the repository's own precedent for a reviewer PASS releasing them in the same commit is `d77ee55` (T-0114, which deleted `queue/LOCKS/Package.swift.lock` alongside the `git mv`). My `git rm queue/LOCKS/package-swift.lock queue/LOCKS/pbxproj.lock` was refused by the permission system as a shared-resource modification, twice, and I did not route around it. `.worktrees/T-0141` was therefore returned to pristine â€” `git status --short` empty, HEAD `5aeaf54`, task file back to blob `7ccc373a`, both locks intact â€” and `.worktrees/rv2-pr88` removed. The verdict stands as PASS; whoever signs off needs to release those two locks in the same commit as the `git mv`, which makes it three paths under `queue/`, not one.
- 2026-09-18T18:05:00Z agent/claude-fable-5-1 (owner): the entry above is the reviewer's, verbatim from its report. Its
  sign-off transition was refused by the permission layer at `git rm queue/LOCKS/package-swift.lock
  queue/LOCKS/pbxproj.lock` (twice) and it reverted everything; this commit performs exactly the transition it
  specified - `state: done`, `reviewer: agent/rv2-pr88`, `git mv` to done/, both locks released as in d77ee55 -
  and nothing else. The verdict is the reviewer's, not the owner's.
