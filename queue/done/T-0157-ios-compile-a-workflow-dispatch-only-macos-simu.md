...
- 2026-09-18T20:21:18Z **Review PASS (round 4) - agent/rv4-pr90 (reviewer; not the owner and not the round-1, round-2 or round-3 reviewer).** PR #90 read at `1f3399f` in a detached `.worktrees/rv4-pr90` (== `origin/task/T-0157`), removed at the end; nothing written in `.worktrees/T-0157` (clean, at `1f3399f`) except this transition; every mutated copy lived in `.artifacts/rv4-pr90/mutants/` (gitignored, outside the tracked tree) and was fed to the check by path. Both acceptance lines reproduce verbatim: `python ops/lib/check-ios-compile-guardrails.py` -> `IOS-COMPILE-GUARDRAILS OK: ios-compile.yml equals the pinned workflow: dispatch-only, contents: read, ['macos-15'], time-boxed, and a build that cannot be skipped or fail green` (exit 0); `--prove-red` -> 26 `[red rc=1]` lines, 3 `[green rc=0]` legitimate spellings (`on:` as a bare string, `runs-on` as a one-element list, a comment added), `[refused rc=2] unreadable YAML: ... ParserError`, `[green rc=0] the shipped file`, then `PROVE-RED OK: 26 mutations red, 3 legitimate spellings green, 0 unexpected result(s)` (exit 0); every mutation applies exactly once, so none proves over a no-op. Round 3's mutant re-applied by hand from the shipped text - `if: false` inserted under the build step's name, nothing else changed - is refused BY NAME: `jobs.simulator-build.steps[2].if: a build step that can be SKIPPED is a green job that compiled nothing (found False)`, exit 1.
  **No blocking finding. Six mutants, each applied ALONE, on the two questions equality leaves open: is `normalise()` too generous, and can the PARSER be made to see the pinned structure while GitHub sees another.** Generosity: `on: workflow_dispatch:` given `inputs: {x: {default: y}}` -> exit 1, `workflow.on.workflow_dispatch: differs from the pinned value (compared by equality)` (folding is limited to a string or a list; a mapping is compared as it is); `runs-on: [macos-15, self-hosted]` does NOT fold, the unwrap being `len == 1` only -> exit 2, `has a shape this check cannot judge: TypeError`; `runs-on: [macos-15-xlarge]` folds and is then refused by the label allowlist -> exit 1, `jobs.simulator-build.runs-on: must be one of ['macos-15'], found 'macos-15-xlarge'`. Divergence: a second YAML document appended after `---` carrying `on: push:` -> exit 2, `cannot read m6.yml: ComposerError`. All four non-zero: no widened trigger, token scope, runner or timeout reached exit 0, and neither did a skippable or failure-tolerant build.
  **RECORDABLE - two mutants reach exit 0, and both are doors GitHub itself closes, so nothing runs.** (1) A DUPLICATE top-level `permissions:` - `contents: write` plus `id-token: write` first, the pinned `contents: read` second - is exit 0, `IOS-COMPILE-GUARDRAILS OK: ... equals the pinned workflow ...`: PyYAML keeps the last mapping, so the parsed document is the pinned one while the file's text carries a write scope, and `raw` is only grepped for `secrets.`, so the check cannot see the shadowed text. GitHub's workflow parser rejects duplicate keys as an invalid workflow file, so the run never starts - fail-closed, and not executed here (a dispatch needs the file on the default branch). (2) A merge key, `permissions:` with `<<: {contents: read}`, is exit 0: PyYAML expands the merge, and here the expansion IS the pinned value, so nothing dangerous passed; GitHub supports neither anchors/aliases nor a `<<` permission scope, so again an invalid workflow rather than a divergent run. Also recordable: the multi-label `runs-on` above exits 2 ("cannot tell") from a caught `TypeError` (a `list` tested against a `set`) rather than exit 1 naming the runner, though the check CAN tell in that case - by reading, a mapping `runs-on: {group: ..., labels: [...]}` takes the same unhashable path; both fail closed. And the `inputs:` refusal prints a path rather than a named guardrail, so a maintainer adding a legitimate dispatch input is told "differs from the pinned value" and must edit `EXPECTED` deliberately - correct, and the intended cost. Record lag: the PR body still measures `e3c7345` (`PROVE-RED OK: 14 mutations red, 2 legitimate spellings green`) while HEAD prints 26/3, and its Review section stops at round 2; round 3 recorded the same lag.
  **EXPECTED against the shipped file.** Equality makes the two identical today by construction and the green run proves it; read against the brief, the pinned scripts are what it requires: `-project apps/ios/ScenicDrive.xcodeproj -scheme ScenicDrive`, `-destination 'generic/platform=iOS Simulator'`, `-derivedDataPath "$GITHUB_WORKSPACE/DerivedData"` (CLAUDE.md's rule; `DerivedData/` ignored at .gitignore:12), `CODE_SIGNING_ALLOWED=NO`, `build | tee "$GITHUB_WORKSPACE/DerivedData/xcodebuild.log"` (one workspace-relative artifact root), `mkdir -p` before the tee, `git status --porcelain --untracked-files=all` under `if: always()`, both `uses:` allowlisted, no commit and no push step.
  **Record hygiene.** agent/rv3-pr90's round-3 FAIL entry is carried in full in the round-4 commit and unedited since; `git diff b736d05 1f3399f -- queue/` has exactly two deletions in the whole history, the two renumbered acceptance lines, so no Log line has ever been rewritten; the round-4 commit is +28/-2 in `queue/` and touches only the check and the task file (`git diff --stat 1f3399f^ 1f3399f`), the workflow being unchanged since round 3. Nothing in the task file or the check claims the workflow has run: the third acceptance line says `NOT RUN, on the record`, and round 4 says it again ("it pins what the file SAYS, not what GitHub does with it").
  **Not run here:** `bash ops/check-pins` and `bash ops/test` locally - on this box the default swift scratch path does not build inside a worktree and `--source-only` drives that build (8+ minutes lost in round 1). CI at this exact head is the evidence, read ONCE, no polling: `gh pr checks 90` -> `core pass 2m13s`, `pins-source-only pass 1m4s` (run 35390500838), the PR's `headRefOid` being `1f3399f`. Also not run: the workflow itself, GitHub's rejection of the two invalid-YAML doors above, and a tab/BOM variant. **STILL OPEN, carried unchanged:** nothing RUNS this check yet (no pin, no CI job) - the follow-up's first item; `Package.resolved` is still uncommitted.
  PASS: `queue/claimed/` -> `queue/done/`, `reviewer: agent/rv4-pr90`, `state: done`. Not merged - the reviewer does not merge.
- 2026-09-18T20:32:12Z **THE WORKFLOW HAS NOW RUN - agent/claude-fable-5-1 (owner), after the merge (PR #90).** GREEN on main, dispatched with gh workflow run ios-compile.yml --ref main: run 35391464739, toolchain step printed Xcode 16.4 / Build version 16F6 / Apple Swift version 6.1.2, the build step ended ** BUILD SUCCEEDED ** with zero compiler errors and one warning (appintentsmetadataprocessor: Metadata extraction skipped), the status step printed ?? apps/ios/ScenicDrive.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved and the artifact ios-compile-35391464739 carries that file and the log. This is the first time anything has compiled apps/ios. RED, because a check seen only green is untested: a throwaway branch probe/ios-compile-red (commit a8f02c7, one appended line assigning a String to an Int in DesignTokens.swift), dispatched through the same workflow: run 35391738972, DesignTokens.swift:104:31: error: cannot convert value of type 'String', ** BUILD FAILED **, Process completed with exit code 65, job conclusion failure; both if: always() steps concluded success and did not mask it. The probe branch was deleted from origin and its worktree removed in the same command; nothing of it is on main. What the first run surfaced - Package.resolved untracked, the default Xcode being 16.4 while the plan requires the iOS 26 SDK (26.0-26.3 are on the image), the guardrail check still run by nothing - is T-0167. What it does NOT prove is unchanged: signing, TestFlight, the 40-minute push-to-phone gate (T-0009, the human).
