...
acceptance:
  - "MEASURED FIRST, each candidate ALONE over the real three-window union (46,231 ways, 79,764 segments, 19,906,560 B = 430.59 B/way; T-0206's Log) with the bytes and B/way quoted as each lands: (a) VACUUM and a SMALLER page_size (schema.py PRAGMAS already sets page_size=4096 and auto_vacuum=NONE explicitly); (b) service ways left out of the corpus (312,645 of the clip's 560,208 filtered ways = 55.8%; T-0207 already scores every service way 0) - RULE what the app loses: the corpus serves saved-drive re-resolution, the hazard strip and segment lookup, so a route that legitimately crosses a service road must still resolve; (c) segments_rtree as a second copy of every bbox; (d) geom_sha256 at 32 B/way and the name column, and osm_features.cls/highway repeated TEXT interned to a class table (plus the osm_features_by_cls and segments_by_way indexes measured); (e) WITHOUT ROWID / integer-packed geometry"
...
- 2026-09-19T16:40:26Z candidates (a) and (d) corrected by agent/claude-fable-5-1 (06:13 panel, CODE lens, fable-grounded on assemble.py:141-150 / schema.py): page_size=4096 is already explicit (schema.py PRAGMAS), so the live knob is VACUUM and a smaller page; the repeated cls/highway TEXT and the two secondary indexes were missing from the list (schema.py:90-91, :101, :121).
