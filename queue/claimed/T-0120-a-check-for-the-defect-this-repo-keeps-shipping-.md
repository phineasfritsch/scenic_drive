...
- 2026-09-08T21:15:00Z reviewer-pr77 returned FAIL with 6 blocking + 7 smaller findings. Closed 1-4, each demonstrated red then green by `.artifacts/red77.py`, which mutates a tracked file in this worktree and restores it byte-identically (compared as bytes, not by re-reading a diff).
- 2026-09-08T21:15:00Z **FINDING 4, the urgent one: this check was firing on CORRECT code.** The right-hand side is read as the rest of the line, so `#expect(a == b, "why")` captured `b, "why")`, and `.rstrip(")").rstrip(",")` cannot strip a message. On the real tree `#expect(Geo.earthRadiusMeters == 6_371_008.8, "WGS84 mean radius")` took this check to exit 1 and P-TEST-02 red - on a shape 7 of the 31 existing assertions use. Added `MESSAGE_TAIL`, which requires the comma (so `== "text"` is untouched) and stops at an array literal's closing bracket (so `== ["a", "b"]` still passes). RED: with `MESSAGE_TAIL.sub` reverted, both new green cases fire with `SELF-REF FAIL: 1 assertion(s) take their expected value from the code under test`.
- 2026-09-08T21:15:00Z **FINDING 2: a crashing matcher passed every negative case.** The `"SELF-REF FAIL" in out` guard was applied to the RED half only. `fired = False` is precisely what a negative case wants, so a matcher replaced by `raise` handed nine of nineteen cases their expected result and the suite exited 0. A negative case must now show the check RAN and was SILENT - exit 0 *and* its own OK line. RED: with `problems_in` replaced by `raise`, **all 22 cases FAIL**, including all 13 greens that previously printed `ok`.
- 2026-09-08T21:15:00Z **FINDING 3: `CASES = []` printed `SELF-REF CASES OK (0 cases)` and exited 0**, and so did a negatives-only suite. `check-self-referential-history.py:137` has had this guard since it was written; this file did not. Added `MIN_RED_CASES = 8` / `MIN_GREEN_CASES = 6`, both directions required - greens alone cannot show the matcher fires, reds alone cannot show it ever stays quiet. RED: emptying CASES gives `SELF-REF CASES FAIL: 0 red and 0 green cases, expected at least 8 and 6`, exit 2.
- 2026-09-08T21:15:00Z **FINDING 1: the function-call lookahead was unpinned**, and the case that claimed to pin it passed either way. The reviewer identified the shape that actually exercises it - a call on the RIGHT-hand side - and it is now a case: `#expect(x <= Geo.distanceMeters(a, b))`. RED: deleting `(?!\s*\()` from `MEMBER` turns it red.
- 2026-09-08T21:15:00Z GREEN after all four: `bash ops/check-tests` -> `SELF-REF OK (31 assertions in 4 test files, 5 types under Sources/)`, exit 0. `check-self-referential-cases.py` -> `SELF-REF CASES OK (22 cases)`. `bash ops/check-pins` -> `ok=12 skipped=0 pending=2 expired=0 failed=0`. `bash ops/queue-check` -> `QUEUE OK (110 tasks)`.
- 2026-09-08T21:15:00Z **NOT FIXED, named rather than quietly left.** Finding 5: `MIN_ASSERTIONS` and `MIN_TYPES` are still never seen red, and the rule-C `\b` is unpinned. Finding 6: no floor guards how many assertions reference a declared type (12 of 31 today). Finding 7: `suppressed` is incremented only when `is_assertion`, so an allowlisted rule-C range finding is silenced while the OK line stays unqualified - which defeats the count's stated purpose. Findings 8-13: a stale exit code quoted in this Log, two undocumented false-negative shapes (`while i < Type.constant`, `stride(from:to:)`), no false-positive section, `len(problems)` printed as "N assertion(s)" when it is per-line, abbreviated SHAs in the history docstring, and `ops/check-tests --case <typo>` silently running the ordinary check and exiting 0. Findings 5 and 6 are the same class as 2 and 3 - a floor that cannot be seen red - and should be the next work on this file.
