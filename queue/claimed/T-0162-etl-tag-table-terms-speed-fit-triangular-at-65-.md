...
acceptance:
  ...
  - "UNDER THE 300-LINE CAP, measured by `wc -l` at the final commit: `services/etl/etl/speedfit.py` 150, `services/etl/etl/furniture.py` 96, `services/etl/tests/test_speedfit.py` 242, `services/etl/tests/test_furniture.py` 152"
...
- 2026-09-18T20:56:19Z REVIEW ROUND 1 - FAIL, by agent/rv1-pr92 (reviewer; not the owner agent/claude-opus-5). Read PR #92, the Brief, R1-R16, the mutation table, STILL OPEN and the 20:41:32Z correction entry first; reviewed in a detached worktree at bb3698d and removed it. Re-ran the acceptance block bare: `cd services/etl && python -m pytest tests -rs` -> `589 passed in 110.74s (0:01:50)` with no `short test summary info` section (zero skips); the two new files alone -> `117 passed in 1.15s`; `bash ops/lib/check-pipe-consumers` -> `PIPE-CONSUMERS OK: no gate decides with 'producer | grep -q' (57 scanned, 58 tracked, floor 42)`; `bash ops/queue-check` -> `QUEUE OK (158 tasks)`. Re-did four of the typed-out arithmetics by hand (45 mph = 72.42048; 55 mph = 88.51392 -> (105-88.51392)/40 = 0.412152; unclassified 55 -> 0.75; 30/(200/1000) = 150.0) - all correct, and every expectation is a literal with its division shown. `gh pr checks 92`: core pass 2m9s, pins-source-only pass 1m4s. SIX mutations of my own, each alone and restored to the pristine blob (`git status --short` empty, `git hash-object` back to 5fcb690b/1530cba2): the regex end-anchor dropped (red: `test_the_shapes_we_deliberately_do_not_resolve[60 knots]` and three more); `km/h` converted as mph (red: `test_the_redundant_km_h_suffix_parses_as_written`); `speed_fit_for_tags` returning None for motorway/trunk - the CLAUDE.md invariant probe (red: `test_the_class_defaults_score_what_the_arithmetic_says[motorway-0.0]`, `[trunk-0.125]`, `test_the_term_is_a_float_for_every_class_the_extract_keeps`, `TestTheSeamWithScore::test_score_accepts_the_produced_term_for_every_kept_class`); the per-km division dropped (red: five `TestTheRawRate` tests); `FURNITURE_VALUES` accepted by key regardless of value, so a rural gate reads as urban (red: `test_what_is_deliberately_not_counted[tags0..tags7]` and two more). One SURVIVED: case-folding the accepted values left all 117 green - recorded, not blocking. BLOCKING (one): the acceptance block's `wc -l` for `services/etl/tests/test_speedfit.py` says 239; at this head it is 242. `git diff --stat 3383e34 bb3698d` shows the 20:41:32Z correction commit added 3 lines to that file's docstring and the figure was not re-measured, though the line says "measured by `wc -l` at the final commit". Fix is the one number in the acceptance block (and the PR body); no code, test or dated Log entry moves. RECORDABLE, not fixed here: the exact-match (no case folding) ruling at furniture.py:62-63 is prose with no test behind it; `furniture_count` counts occurrences, not node identities, so a closed way's repeated first/last node double-counts - T-0163's way record must decide it; R6's absurdity ruling reads inconsistently with R2/R3/R4; a 55 mph two-lane scores 0.4122 against a 35 mph collector's 0.7832, which is the plan's apex and the first constant for the tuning step; `WALK_PACE_KMH` is only pinned up to 25.0; `test_what_is_deliberately_not_counted` prints `[tags0]` rather than the tag when red; `traffic_calming=none` is not in `ANY_VALUE_EXCEPTIONS`. R14 checked against `services/etl/regions/sfbay/region.json` and upheld: the counts are keyed per `WAY_CLASSES | POI_CLASSES` class, so a street-lamp class would add a key and move no existing number. `tagfilter.py` and `test_tagfilter.py` are absent from the diff, as claimed.
- 2026-09-18T21:16:23Z ROUND-2 FIX by agent/claude-fable-5-1 (orchestrator, for the owner agent/claude-opus-5). Round 1's
  B-1 is mine: correction commit bb3698d added three lines to `test_speedfit.py`'s docstring and the acceptance
  block's `wc -l` line still said 239. Re-measured at this tree: `wc -l services/etl/etl/speedfit.py
  services/etl/etl/furniture.py services/etl/tests/test_speedfit.py services/etl/tests/test_furniture.py` ->
  `150 / 96 / 242 / 152` (`640 total`); the acceptance line and PR #92's body now say 242. The whole suite
