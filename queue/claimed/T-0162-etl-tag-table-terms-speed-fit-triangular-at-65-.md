...
acceptance:
  ...
  - "`speed_fit` IS THE PLAN'S TRIANGLE AND ITS FEET ARE A RECORDED RULING, not a computed one. `services/etl/etl/speedfit.py` carries `APEX_KMH = 65.0` (plan:89), `LOWER_FOOT_KMH = 25.0` and `UPPER_FOOT_KMH = 105.0` (R1), and every expected value in `tests/test_speedfit.py` is typed out with its division shown in the comment above it - 1.0 at the apex, 0.0 at both feet, 0.5 at BOTH 45 and 85 ((45-25)/40 and (105-85)/40), 0.875 at 60 and at 70, 0.25 at 35 and at 95. No expectation in the file is obtained by calling the function under test or by re-deriving it from the module's constants at test time"
